...
The biggest gap. `bootstrap-memory-bank-v2-contract.md` describes seeding `.memory-bank-v2/`, but nothing runs it. No `npx torque-coding init --v2` path. Users would copy `templates/machine/*` by hand, which nobody will do.
...
**Done when**: either `skills/memory-bank/update-active-context-v2/SKILL.md` exists and all state skills reference it, or the v1 skill is extended with v2-aware sections documented in both places.

### 3. `templates/machine/toc.md`
...
**Done when**: a v2-specific `toc.md` template is in `templates/machine/` and the bootstrap contract copies it.

### 4. Getting-started doc

`README.md` is doctrine. There's no "clone, install, run your first task" page for a developer who's never seen this. v1's `README.md` fills that role for v1; v2 has no equivalent.
...
**Done when**: `claude-rules/compaction.v2.md` exists with explicit compact/restore procedure, and `AGENTS.v2.md` cross-references it.
...
*Open this alongside `README.md` (doctrine) and `V2-README.md` (root pointer) to see where v2 stands and what's next.*
