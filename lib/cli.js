...
async function run() {
  ...
  if (stagedSkills.length > 0) {
    ...
    console.log(`  4. Commit ${cyan('AGENTS.md')}, ${cyan('.agent/')}, ${cyan('docs/memory-bank/')}, ${cyan('.memory-bank/')}, and ${cyan('.agent-playbook.json')}.\n`);
  } else {
    console.log(`  3. Commit ${cyan('AGENTS.md')}, ${cyan('.agent/')}, ${cyan('docs/memory-bank/')}, ${cyan('.memory-bank/')}, and ${cyan('.agent-playbook.json')}.\n`);
  }
...
async function update() {
  ...
  if (!manifest) {
    throw new Error(
      'No playbook manifest found and no installed skill set could be inferred. Run `agent-playbook init` once to seed .agent-playbook.json before using update.'
    );
  }
