...
const { convertSkillDir, convertInjectedSkillRefs } = require('./convert-skill');
...
/**
 * Copy core operating model for Claude Code target.
 *
 * - CLAUDE.md goes to project root
 * - Rule files go to .claude/rules/
 * - Skills are converted from generic format to Claude Code native SKILL.md format
 *   and placed in .claude/skills/
 *
 * Options:
 * - copyCoreDoc: whether to copy CLAUDE.md to project root (default true)
 * - targetDir: project root directory (required when copyCoreDoc is true)
 */
function copyCoreClaude(pkgRoot, targetDir, options = {}) {
  const {
    copyCoreDoc = true,
    includeBootstrapArtifacts = true,
  } = options;
  ...
  // CLAUDE.md goes to project root
  if (copyCoreDoc) {
    safe('Copy CLAUDE.md to project root', () =>
      fs.copyFileSync(
        path.join(pkgRoot, 'agent', 'CLAUDE.md'),
  ...
  safe('Create .claude/rules directory', () =>
    fs.mkdirSync(rulesDir, { recursive: true })
  );

  const rulesSrc = path.join(pkgRoot, 'agent', 'claude-rules');
  for (const entry of fs.readdirSync(rulesSrc, { withFileTypes: true })) {
    if (entry.name === '.DS_Store') continue;
    if (!entry.isFile()) continue;
    safe(`Copy rule: ${entry.name}`, () =>
      fs.copyFileSync(
  ...
  if (includeBootstrapArtifacts) {
    // Bootstrap contract and templates go to docs/memory-bank/
    const docsDir = path.join(targetDir, 'docs', 'memory-bank');
    safe('Create docs/memory-bank directory', () =>
      fs.mkdirSync(docsDir, { recursive: true })
    );
  ...
  // State machine skills — convert from generic to Claude Code format
  const skillsDir = path.join(claudeDir, 'skills');
  safe('Convert state machine skills for Claude Code', () => {
    const dest = path.join(skillsDir, 'state-machine');
  ...
  // Memory bank skills — convert from generic to Claude Code format
  safe('Convert memory bank skills for Claude Code', () => {
    const dest = path.join(skillsDir, 'memory-bank');
    fs.rmSync(dest, { recursive: true, force: true });
    convertSkillDir(
      path.join(pkgRoot, 'skills', 'memory-bank'),
      dest
...
/**
 * Copy and convert skills from a skill pack directory for Claude Code target.
 * Returns the number of skills converted.
 */
function copySkillPackClaude(pkgRoot, claudeDir, packDirName) {
  ...
  for (const entry of fs.readdirSync(packDir, { withFileTypes: true })) {
    ...
    safe(`Convert skill pack: ${packDirName}/${entry.name}`, () => {
      fs.rmSync(target, { recursive: true, force: true });
      convertSkillDir(skillDir, target);
    });
    count++;
  }
...
/**
 * Copy and convert an optional skill for Claude Code target.
 */
function copyOptionalSkillClaude(pkgRoot, claudeDir, skillName) {
  const src = path.join(pkgRoot, 'optional-skills', skillName);
  const dest = path.join(claudeDir, 'skills', skillName);

  if (!fs.existsSync(src)) {
    throw new Error(`Optional skill not found: ${skillName}\n  Expected at: ${src}`);
  }

  safe(`Convert optional skill: ${skillName}`, () => {
    fs.rmSync(dest, { recursive: true, force: true });
    convertSkillDir(src, dest);
  });
}

/**
 * Merge injected-skills files for Claude Code target.
 *
 * Same logic as mergeInjectedSkills but targets .claude/skills/ paths
 * and converts .agent/skills/ references to .claude/skills/.
 */
function mergeInjectedSkillsClaude(pkgRoot, claudeDir, selectedPackDirs) {
  const targets = [
    {
      type: 'plan',
      sourceFile: 'plan.injected-skills.md',
      destFile: path.join(
        claudeDir, 'skills', 'state-machine', 'writing-plans',
    ...
    },
    {
      ...
      destFile: path.join(
        claudeDir, 'skills', 'state-machine', 'build-execution',
        'references', 'injected-skills.md'
      ),
    },
  ];
  ...
  for (const target of targets) {
    ...
    for (const dir of selectedPackDirs) {
      ...
      // Convert .agent/skills/ refs to .claude/skills/
      content = convertInjectedSkillRefs(content);
    ...
    safe(`Write merged ${target.type} injection file (Claude)`, () =>
...
- Memory bank bootstrapped via \`torque-coding init\`
...
function manifestPath(targetDir) {
  return path.join(targetDir, '.torque-coding.json');
}
...
function parseManifest(filePath) {
  try {
    ...
    throw new Error(`Invalid torque-coding manifest: ${filePath}\n  ${err.message}`);
  }
}
