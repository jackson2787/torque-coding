...
**What it serves**: BUILD (branch work), QA (verification against remote CI), DEBRIEF (PR creation on archive), ESCALATE (cross-session fallback)

**Server**: [GitHub Official MCP Server](https://github.com/github/github-mcp-server) — Remote (GitHub-hosted)

Your state machine works against a git working tree. BUILD applies changes; QA verifies them; DEBRIEF closes the loop and can open a PR or push branches as part of archival. All of this benefits from first-class GitHub access.
...
**State machine touchpoints**: BUILD (branch work, diff generation), QA (remote CI checks if applicable), DEBRIEF (PR creation, archive commit)

---

### Probe — Code Search MCP Server

**What it serves**: The Six Sacred Rules, PLAN state (reuse analysis), PLAN-CONTEXTUALIZE (integration-point mapping), Bootstrap Memory Bank evidence sweep
...
**State machine touchpoints**: PLAN (analysing existing code, reuse strategy), PLAN-CONTEXTUALIZE (pasting current file state, finding integration points), Bootstrap Memory Bank evidence sweep, Sacred Rule #1 (reuse-over-creation search)
...
**What it serves**: PLAN (understanding data model), PLAN-CONTEXTUALIZE (schema/policy pasting), BUILD (extending backend), QA Check 5 (constitution boundaries involving data/auth)
...
This directly supports the "no ignoring existing architecture" sacred rule. The agent can check the real schema before proposing changes, verify RLS/auth policies are in place during QA's constitution-boundary check, and understand the actual API surface when planning frontend work.
...
**State machine touchpoints**: PLAN (data model analysis), PLAN-CONTEXTUALIZE (schema citations), BUILD (backend integration), QA (constitution boundary check on data/auth)
...
Replaces brute-force file loading with query-based retrieval over your `.memory-bank-v2/` directory. The agent asks "find decisions about caching" and gets back the relevant section rather than loading all of `operational-context.md` or an archived task folder.

**When to build**: When your Memory Bank grows large enough that loading whole files wastes meaningful context. Rules of thumb:
- Total machine-side MB content exceeds 15-20K tokens
- `operational-context.md` has more than 20 directive entries
- `human/tasks/` spans more than 4-5 months
- The agent is loading files just to find one section

**What it does**: Indexes MB files by heading structure, supports keyword and (optionally) semantic search, returns sections with source paths. Also supports the `mb_read_section` pattern for following citations like `operational-context.md#Active-Patterns`.

**Architecture**: Local process, reads from `.memory-bank-v2/` directory, no external dependencies. Travels with the repo. Falls back gracefully — if the server isn't running, the agent reads files directly.

**State machine touchpoints**: Session Startup (machine-side load set), Compaction Recovery, PLAN (doctrine lookup), PLAN-CONTEXTUALIZE (pattern citations), DEBRIEF (archival + operational-context diff proposals)
...
This maps directly to the structure of `qa-report.md` (written by the `qa` skill) and eliminates parsing failures that can cause false stalls.
...
**State machine touchpoints**: QA (all six checks), cycle counting against `limits.md` hard caps
...
MCP servers are configured in your project's MCP config file (`.brain/mcp.config.json` or `.mcp.json`, attached at the Session Startup step in AGENTS.md). The agent reads this at session startup.
...
|--------|------|---------------|--------------|
| [GitHub Remote](https://github.com/github/github-mcp-server) | 1 — Core | BUILD, QA, DEBRIEF (PR/archive) | Immediately |
| [Probe](https://github.com/probelabs/probe) | 1 — Core | Six Sacred Rules, PLAN, PLAN-CONTEXTUALIZE, bootstrap evidence sweep | Immediately |
| Backend (Supabase, etc.) | 2 — Stack | PLAN, PLAN-CONTEXTUALIZE, BUILD, QA Check 5 | When project has a backend |
| Framework (Vercel, etc.) | 2 — Stack | BUILD (framework patterns), QA (framework verification) | When using domain-tuned variant |
| Memory Bank (custom) | 3 — Custom | Session Startup, MB reads, DEBRIEF writes | When MB exceeds ~15K tokens |
| Project QA (custom) | 3 — Custom | QA state (all six checks) | When QA parsing becomes unreliable |
